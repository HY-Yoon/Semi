...
<link rel="stylesheet"
	href="${pageContext.request.contextPath }/css/board_qa/content.css">
	...
	<div class="main">
		<div class="left">
			<div class="content">
				<div class="title">
					<img
						src="${pageContext.request.contextPath}/images/common/icon/question.svg" />
					${bvo.title }
				</div>
				<div class="middletop">
					상태
					<c:choose>
						<c:when test="${bvo.keyword }=='답변대기'">
							<span style="color: gray"> ${bvo.keyword } </span>
						</c:when>
				...
				<div class="middlebottom">${bvo.content }</div>
				<div class="bottom">
					<div class="hashtag">
					...
					<div class="contentinfo">
						...
						<%=UserReportController.getHTML(request, Integer.parseInt(request.getParameter("mnum")))%>
					</div>
					<div class="contentud">
						<c:choose>
							<c:when
								test="${sessionScope.sessionMnum eq requestScope.bvo.mnum }">
								<input type="button" value="수정"
									onclick="location.href='${pageContext.request.contextPath}/html&jsp/board_qa/updateboard_qa?anum=${bvo.anum}'">
								<input type="button" values="삭제"
									onclick="location.href='${pageContext.request.contextPath}/html&jsp/board_qa/deleteboard_qa?anum=${bvo.anum}'">
							</c:when>
			...
			<div class="comment">
				<div class="commcnt">${commcnt }개의답변</div>
				<div id="qacomm" name="qacomm" class="qacomm"></div>
			</div>
		</div>
		<div class="right">
			<div class="amemberinfo">
				<div id="info">${bvo.nick }연령대,관심지역, 등급</div>
			</div>
			<div class="status">
				<c:choose>
					<c:when test="${bvo.keyword eq '채택완료'}">
						<div class="selcomplete">답변 채택이 완료되었습니다.</div>
					</c:when>
					<c:when test="${bvo.keyword eq '답변대기'}">
						<c:choose>
							<c:when test="${empty sessionScope.sessionID}">
								<input type="button" value="답변하기"
									onclick="window.open('${pageContext.request.contextPath}/html&jsp/board_qa/loginPopup.jsp','login','width=450, height=250, top= resizable=no, scrollbars=no')">
							</c:when>
							<c:when test="${!empty sessionScope.sessionID}">
								<input type="button" value="답변하기"
									onclick="location.href='${pageContext.request.contextPath}/html&jsp/board_qa/comminsert_qa?anum=${bvo.anum}'">
							</c:when>
						</c:choose>
					</c:when>
				</c:choose>
