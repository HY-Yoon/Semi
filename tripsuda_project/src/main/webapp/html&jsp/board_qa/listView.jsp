...
<link rel="stylesheet"
	href="${pageContext.request.contextPath }/css/board_qa/list_qa.css">
...
<script type="text/javascript">
	...
	function boardList(pageNum, selc) {
			...
			if (xhr.readyState == 4 && xhr.status == 200) { //응답하고 응답 성공했을 때 명령 처리하기
				...
				for (let i = 0; i < list.length; i++) {
					...
					listbox.onclick = function() {
						location.href = "${pageContext.request.contextPath}/html&jsp/board_qa/content?anum="
								+ anum;
					}
				...
				let pageInner = "";
				if (startPage > 6) {
					pageInner += "<a href='javascript:boardList("
							+ (startPage - 1) + ")'>이전페이지</a>";
				}
				for (let i = startPage; i <= endPage; i++) {
					if (i == pageNum) {
						pageInner += "<a href='javascript:boardList(" + i
								+ ")'>[" + i + "]</a>";
					} else {
						pageInner += "<a href='javascript:boardList(" + i
		...
		xhr.open('post',
				'${pageContext.request.contextPath}/html&jsp/board_qa/list',
				true);
...
<body>
	<div class="main">
		<div class="qa_orderby">
			<span>정렬</span> <select id="sortselect" name="sortselect"
				onchange="sortsel()">
				<option value="1">등록순</option>
				<option value="2">댓글순</option>
			</select> <input type="button" value="답변을 기다리는 질문만 보기" onclick="sortsel()">
		</div>
